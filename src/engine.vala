...
class SkkEngine : IBus.Engine {
    ...
    construct {
        ...
        context.notify["preedit"].connect (() => {
                ...
                if (context.preedit.has_prefix ("▼")) {
                    var start = "▼".length;
                    var end = context.preedit.last_index_of ("*");
                    if (end < 0) {
                        end = context.preedit.length - 1;
                    }
                    text.append_attribute (IBus.AttrType.FOREGROUND,
                                           rgb (0, 0, 0),
                                           start,
                                           end);
                    text.append_attribute (IBus.AttrType.BACKGROUND,
                                           rgb (180, 238, 180),
                                           start,
                                           end);
                }
    ...
    static uint rgb (uint8 r, uint8 g, uint8 b) {

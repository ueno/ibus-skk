...
class Setup : Object {
    ...
    Gtk.ComboBox rom_kana_rule_combobox;
    ...
    public Setup (Preferences preferences) {
        ...
        builder.add_from_file (Path.build_filename (Config.SETUPDIR,
        ...
        object = builder.get_object ("rom_kana_rule_combobox");
        assert (object != null);
        rom_kana_rule_combobox = (Gtk.ComboBox) object;
        ...
        rom_kana_rule_combobox.set_model (model);
        try {
            var dir = Dir.open (Config.ROMKANADIR);
            string? name;
            while ((name = dir.read_name ()) != null) {
                if (name.has_suffix (".json")) {
                    Gtk.TreeIter iter;
                    model.append (out iter);
                    model.set (iter, 0, name[0:-5]);
                }
            }
        ...
        renderer = new Gtk.CellRendererText ();
        rom_kana_rule_combobox.pack_start (renderer, false);
        rom_kana_rule_combobox.set_attributes (renderer, "text", 0);
    ...
    void populate_dictionaries_treeview () {
        ...
        foreach (var str in strv) {
            Gtk.TreeIter iter;
            model.append (out iter);
            model.set (iter, 0, new PList (str));
        }
    }
        ...
        if (dict_dialog.run () == Gtk.ResponseType.OK) {
            ...
            if (text == "System") {
                ...
                if (host.length > 0 && port > 0) {
                    plist = new PList ("type=server,host=%s,port=%d".printf (
                                           PList.escape (host),
                                           port));
                }
            }
    ...
    void load () {
        ...
        variant = preferences.get ("rom_kana_rule");
        assert (variant != null);
        var model = (Gtk.ListStore) rom_kana_rule_combobox.get_model ();
        Gtk.TreeIter iter;
        if (model.get_iter_first (out iter)) {
            ...
            do {
                ...
                if (rule == _rule) {
                    rom_kana_rule_combobox.set_active_iter (iter);
                    break;
                }
            } while (model.iter_next (ref iter));
        }
    ...
    void save () {
        ...
        Gtk.TreeIter iter;
        if (rom_kana_rule_combobox.get_active_iter (out iter)) {
            var model = (Gtk.ListStore) rom_kana_rule_combobox.get_model ();
            string rule;
            model.get (iter, 0, out rule, -1);
            preferences.set ("rom_kana_rule", rule);
        }
        preferences.save ();
    }
